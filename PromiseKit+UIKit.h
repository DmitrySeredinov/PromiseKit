...
@interface UIImageView (PromiseKit)

/**
 Obviously your promise should return an image, if it doesn't
 the promise will fail. Duh.
**/
...
/**
 Show the alertView, then handler is called for everything that
 isn't a cancel button push. The then handler parameter is an
 NSNumber that is the index of the button that was pushed.
**/
- (Promise *)promise;
